...
class LocalModeStorageService {
  ...
  static const String _subscriptionContentKey = 'local_mode_subscription_content';
  ...
  /// 保存订阅内容
  static Future<void> saveSubscriptionContent(List<SearchResource> resources) async {
    final prefs = await SharedPreferences.getInstance();
    final jsonList = resources.map((r) => r.toJson()).toList();
    final jsonString = jsonEncode(jsonList);
    await prefs.setString(_subscriptionContentKey, jsonString);
  }

  /// 获取订阅内容
  static Future<List<SearchResource>> getSubscriptionContent() async {
    final prefs = await SharedPreferences.getInstance();
    final jsonString = prefs.getString(_subscriptionContentKey);
  ...
  /// 清除订阅内容
  static Future<void> clearSubscriptionContent() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove(_subscriptionContentKey);
  }
  ...
  static Future<void> saveFavorite(FavoriteItem favorite) async {
    ...
    // 移除相同的收藏项（如果存在）
    favorites.removeWhere((f) => f.source == favorite.source && f.id == favorite.id);
  ...
  static Future<void> clearAllLocalModeData() async {
    ...
    await clearSubscriptionContent();
